...
var paths = {
  // images paths
  images: {
    src: "src/**/*.{ico,jpeg,jpg,png,gif,webp}",
  },
  ...
  html: {
    src: "src/**/*.html",
    execlud: ["src/html-partials{,/**}"],
  },

  // sass or css file paths
  customStyles: {
    src: "src/**/*.{scss,css}",
    execlud: ["src/**/_*", "src/assets/styles/sass{,/**}", "src/assets/styles/vendors/**/*"],
  },

  vendorStyles: {
    src: "src/**/*.{scss,css}",
    execlud: ["src/assets/styles/sass{,/**}", "src/assets/styles/style{.css,.scss}"],
  },
      ...
      .pipe(
        flatMap(function (stream, file) {
          ...
          if (process.platform == "win32") {
            ...
            var route = "../".repeat(
              (file.path.split("src\\").pop().match(/\\/g) || []).length
            );
...
function watch_files() {
  ...
  const mainWatcher = watch("src/**/*");
  mainWatcher.on("all", function (event, path) {
    if (event == "unlink" || event == "unlinkDir") {
      if (process.platform == "win32") {
        var distPath = path.replace("src\\", `${outputDir}\\`);
      } else {
        var distPath = path.replace("src/", `${outputDir}/`);
      }
